...
# Mount iso
mkdir -p ~/iso
sudo mount -r "$LOCATION" ~/iso

kvm -no-reboot -name auto-inst-test -nographic -m 2048 \
-drive file=${DISK_FILE},format=qcow2,cache=none,if=virtio \
-cdrom ${LOCATION} \
